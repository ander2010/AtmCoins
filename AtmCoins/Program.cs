...
namespace AtmCoins
{
   ...
   {"1",10},
        ...
        static void Main(string[] args)
        {
            ...
            string pattern = @"(R|Q)|(W ([$]?[0-9]))|(I ([$]?[0-9]){1,6})";
            ...
            while (outOfProgram != "Q")
            {
                ...
                try
                {
                    if (!CheckRegularExpresion(option, rgx))                    
        ...
        {
            int debitRetire = withdraw.Trim().Contains("$") ? int.Parse(withdraw.Trim().Replace("$", "")) : int.Parse(withdraw.Trim());
        ...
        public static int Selecction(int dif)
        {
            ...
            foreach (var item in BillsProperty) //foreach of the available coins          
            {
                if (item.Value > 0) //if that kind of coins there is money
                {
                    ...
                    {
                        betterResultSelection = int.Parse(item.Key); //saving the ideal coin                      
